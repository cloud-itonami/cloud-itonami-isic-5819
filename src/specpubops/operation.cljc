(ns specpubops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (SpecPubAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the SpecPubGovernor (:govern)
...
(defn build
  ...
  `specpubops.store/Store`).
  opts:
    :advisor      -- a `specpubops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SpecPubAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; SpecPubGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
