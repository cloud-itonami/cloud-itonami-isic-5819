...
(defn -main [& _]
  (let [db (store/seed-db)
    ...
    (let [r (exec-op actor "t1" {:op :log-production-record :title-id "title-1"
                                  :patch {:design-status "final art approved" :print-run 20000}} coordinator-phase-1)]
      (println r)
    ...
    (println (exec-op actor "t2" {:op :log-production-record :title-id "title-1"
                                  :patch {:finish-spec "matte-uv-coated" :print-run 20000}} coordinator-phase-3))
    ...
    (println (exec-op actor "t3" {:op :schedule-production-operation :title-id "title-1"
                                  :patch {:stage "proofing" :date "2026-08-01"}} coordinator-phase-3))
    ...
    (println (exec-op actor "t4" {:op :coordinate-distribution :title-id "title-1"
                                  :patch {:channel "wholesale" :ship-date "2026-09-15"}} coordinator-phase-3))
    ...
    (let [r (exec-op actor "t5" {:op :flag-content-concern :title-id "title-1"
                                 :patch {:concern "possible trademark similarity to an existing seasonal-character property" :confidence 0.92}} coordinator-phase-3)]
      (println r)
    ...
    (println (exec-op actor "t6" {:op :log-production-record :title-id "title-99"
                                  :patch {:design-status "unknown"}} coordinator-phase-3))
    ...
    (println (exec-op actor "t7" {:op :log-production-record :title-id "title-3"
                                  :patch {:design-status "draft"}} coordinator-phase-3))
    ...
    (println "\n== log-production-record title-1, advisor drifts into editorial-content/legal-clearance scope -> HARD hold, permanent ==")
